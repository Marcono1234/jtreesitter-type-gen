...
public sealed interface TypedNode permits NodeContainedA, NodeContainedB, NodeSingleTypeNamed, NodeMultiTypeNamed, NodeMultiTypeNamed$Child {
  /**
...
public final class NodeContainedA implements TypedNode, NodeMultiTypeNamed$Child {
  /**
...
public final class NodeContainedB implements TypedNode, NodeMultiTypeNamed$Child {
  /**
   * Type name of this node, as defined in the grammar.
   */
...
public final class NodeSingleTypeNamed implements TypedNode {
  ...
  public static Stream<NodeSingleTypeNamed> findNodes(TypedNode startNode) {
    ...
    var query = language.query(queryString);
    var stream = query.findMatches(startNodeUnwrapped);
    return stream.flatMap(m -> m.findNodes(captureName).stream()).map(NodeSingleTypeNamed::fromNodeThrowing).onClose(query::close);
  }
...
public sealed interface NodeMultiTypeNamed$Child extends TypedNode permits NodeContainedA, NodeContainedB {
}
...
public final class NodeMultiTypeNamed implements TypedNode {
   ...
   * </ul>
   */
  public @Nullable NodeMultiTypeNamed$Child getChild() {
    var fieldNames = new String[] {};
    var children = NodeUtils.getNonFieldChildren(node, fieldNames, true);
    var namedMapper = NodeMultiTypeNamed$Child.class;
    var childrenMapped = NodeUtils.mapChildren(children, namedMapper, null);
    return NodeUtils.optionalSingleChild(childrenMapped);
  }
  ...
  public static Stream<NodeMultiTypeNamed> findNodes(TypedNode startNode) {
    ...
    var query = language.query(queryString);
    var stream = query.findMatches(startNodeUnwrapped);
    return stream.flatMap(m -> m.findNodes(captureName).stream()).map(NodeMultiTypeNamed::fromNodeThrowing).onClose(query::close);
  }
