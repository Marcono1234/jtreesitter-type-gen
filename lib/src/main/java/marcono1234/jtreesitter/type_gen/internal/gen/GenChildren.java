...
class GenChildren {
    ...
    /**
     * Names of all fields of the enclosing node; currently needed as workaround to get all non-field named
     * children, see code generated by {@link NodeUtilsGenerator}.
     */
    // TODO: Should this use the field name constants, instead of directly using the field names?
    //   but might complicate code, and maybe NodeUtilsGenerator code is refactored anyway to not require list of
    //   field names anymore in the future
    private final Collection<String> allFieldNames;

    protected GenChildren(String getterName, GenChildType type, boolean multiple, boolean required, Collection<String> allFieldNames) {
    ...
    protected void addGetChildrenStatement(MethodSpec.Builder methodBuilder, CodeGenHelper codeGenHelper, String nodeJavaFieldName, String childrenVarName) {
        ...
        CodeGenHelper.addStringArrayVar(methodBuilder, fieldNamesVar, allFieldNames);
        ...
        methodBuilder.addStatement("var $N = $T.$N($N, $N, true)", childrenVarName, nodeUtils.className(), nodeUtils.methodGetNonFieldChildren(), nodeJavaFieldName, fieldNamesVar);
    }
    ...
    public static GenChildren create(String parentTypeName, GenRegularNodeType enclosingNodeType, ChildType childTypeRaw, Collection<String> allFieldNames, NodeTypeLookup nodeTypeLookup, NameGenerator nameGenerator, Consumer<GenJavaType> additionalTypedNodeSubtypeCollector) {
        ...
        return new GenChildren(getterName, childType, multiple, required, allFieldNames);
    }
}
